/*
* @file TPRequestHandler.hpp Trigger matching mechanism for TP frames. 
*
* This is part of the DUNE DAQ , copyright 2021.
* Licensing/copyright details are in the COPYING file that you should have
* received with this code.
*/
#ifndef UDAQ_READOUT_SRC_TPREQUESTHANDLER_HPP_
#define UDAQ_READOUT_SRC_TPREQUESTHANDLER_HPP_

#include "ReadoutIssues.hpp"
#include "DefaultRequestHandlerModel.hpp"

#include "dataformats/tp/TPFrame.hpp"
...
class TPRequestHandler : public DefaultRequestHandlerModel<types::TP_SUPERCHUNK_STRUCT> {
public:
  explicit TPRequestHandler(const std::string& rawtype,
                             std::atomic<bool>& marker,
                             std::function<size_t()>& occupancy_callback,
                             std::function<bool(types::TP_SUPERCHUNK_STRUCT&)>& read_callback,
                             std::function<void(unsigned)>& pop_callback,
                             std::function<types::TP_SUPERCHUNK_STRUCT*(unsigned)>& front_callback,
                             std::unique_ptr<appfwk::DAQSink<std::unique_ptr<dataformats::Fragment>>>& fragment_sink)
  : DefaultRequestHandlerModel<types::TP_SUPERCHUNK_STRUCT>(rawtype, marker,
      occupancy_callback, read_callback, pop_callback, front_callback, fragment_sink)
  {
    ERS_INFO("TPRequestHandler created...");
    data_request_callback_ = std::bind(&TPRequestHandler::tpc_data_request, 
  ...
  void conf(const nlohmann::json& args) override
  {
    ...
    DefaultRequestHandlerModel<types::TP_SUPERCHUNK_STRUCT>::conf(args);
...
protected:
  ...
  tpc_data_request(dfmessages::DataRequest dr, unsigned delay_us) {
    ...
    dataformats::TPHeader front_th = *(reinterpret_cast<const dataformats::TPHeader*>( front_callback_(0) ));
    uint_fast64_t start_win_ts = dr.trigger_timestamp - dr.window_offset;
    uint_fast64_t last_ts = front_th.timestamp();
    ...
    ERS_DEBUG(2, "TPC (TP frame) data request for " 
    ...
    if ( rres.result_code != ResultCode::kFound ) {
      ...
      //auto fromheader = *(reinterpret_cast<const dataformats::TPHeader*>(front_callback_(num_element_offset)));
...
private:
  ...
  const size_t tp_frame_size_ = 464;
  const uint_fast8_t frames_per_element_ = 12;
  const size_t element_size_ = tp_frame_size_ * frames_per_element_;
...
#endif // UDAQ_READOUT_SRC_TPREQUESTHANDLER_HPP_
